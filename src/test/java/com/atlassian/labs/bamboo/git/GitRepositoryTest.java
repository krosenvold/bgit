...
public class GitRepositoryTest {
    ...
    public void testClone() throws IOException, JavaGitException {
        ...
        assertEquals("Repository should be on feature1 branch", "feature1", gitRepository.gitStatus(sourceDir)
                .getName());
    }
    ...
    public void testCollectChangesFromNonExistingSha1() throws IOException, JavaGitException, RepositoryException {
        ...
        final String s = gitRepository.detectCommitsForUrl(HardCodedRepo.NONEXISTANT_SHA1.getSha().getSha(), results,
                sourceDir, "UT-KEY");
        assertEquals(HardCodedRepo.COMMIT_Merge_aBranch_featureDefault.getSha().getSha(), s);
        assertEquals(10, results.size());// This is a bit of a weird assert since I do not exactly
                                         // understand why it gives me 10 items.
    ...
    public void testHistoryWithMergeCommit() throws IOException, JavaGitException, RepositoryException {
        ...
        List<com.atlassian.bamboo.commit.Commit> results = new ArrayList<Commit>();
        gitRepository.detectCommitsForUrl(HardCodedRepo.getFristCommitInBranch().getSha().getSha(), results, sourceDir,
                "UT-KEY");
    ...
    public void testNonLinearHistory() throws IOException, JavaGitException, RepositoryException {
        ...
        // Method collectNodesInRealGitLogOrder does not do it properly -- yet. Need to check both
        // date and sha1
        results = new ArrayList<Commit>();
